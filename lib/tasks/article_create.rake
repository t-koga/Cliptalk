namespace :article_create do
  ...
  desc "dummy articles create in garbage"
  task :create_garbage, [:articles, :group_id, :user_id, :room_id] => :environment do |task, args|
    articles = args[:articles].to_i
    group_id = args[:group_id].to_i
    user_id = args[:user_id].to_i
    room_id = args[:room_id].to_i
    ...
    articles.times do |t|
      article = Article.new(
        ...
        is_destroyed: true)
      if article.save
        puts "Success garbageclip#{t} save!"
      else
        puts "Failed garbageclip#{t} save."
      end
    end
  end
end
